...
<div class="container">
    <div class="row">
        <div class="col-lg-8 animated delay-1s fadeIn">
            ...
            <p>Опубликовано:
                <?php echo date('j F G:i | Y', strtotime($separate_post['datetime'])); ?>
            </p>
            <hr>
            ...
            <!-- Если сессия Админ, то отобрази кнопку "Редактировать" -->
            <?php if (isset($_SESSION['admin'])) { ?>
                <a href="../app/admin/edit.php?post_id=<?= $separate_post['id'] ?>" class="btn btn-primary"
                 style="float:right;background-color:#5fb053">Редактировать</a>
             <?php } ?>
             <hr>
             <!-- Форма с добавлением комментария к статье -->
             <?php if (isset($_SESSION['admin'])) { ?>
            ...
            <?php foreach ($result as $res) { ?>
                <div class="media mb-4">
                    <img class="d-flex mr-3 rounded-circle" src="https://img.icons8.com/nolan/48/000000/user.png"
                    alt="">
                    <div class="media-body">
                        <h5 class="mt-0"><?php echo $res['name']; ?>
                        <!-- Вывод даты и времени, в которое был оставлен комментарий -->
                        <span class="datetime">
                            <?php echo date('j F G:i | Y', strtotime($res['datetime'])); ?>
                        </span>
                            <!-- Если сессия Админ, то предоставь для него возможность
                                удалять комментарии, которые оставили гости -->
                                <?php if (isset($_SESSION['admin'])) { ?>
                                    <a href="post.php?post_id=<?= $separate_post['id'] ?>&del_comment=<?= $res['id'] ?>"
                                     class="guest" style="color: gray;font-weight:100;font-size:14px;float: right;">Удалить</a>
                                 <?php } ?>
                             </h5>
                             <?php echo $res['text']; ?>
                             <hr>
                            ...
                            <div class="col-lg-6">
                                <ul class="list-unstyled mb-0">
                                    <?php foreach ($categories
                                        ...
                                        as $category){ ?>
                                            <li><a href="category.php?id=<?= $category["id"] ?>"><?= $category["title"] ?></a>
                                                <li>
                                                <?php } ?>
                                            </div>
                                        </div>
